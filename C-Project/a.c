...
void  tingLesData(struct Ting* ting)  {
    ...
    }while(enumFraChar(enhet) == ugyldig);
    ting->enhet = enumFraChar(enhet);
}
...
void  tingSkrivData(const struct Ting* ting)  {
    ...
    printf("Navn: %s\t Antall: %f\t Pris: %f\t Enhet:",
    ting->navn, ting->antall, ting->pris);
    switch (charFraEnum(ting->enhet)) {
        case 'C':  printf("cm");        break;
        case 'M':  printf("m");         break;
        case 'K':  printf("kvm");       break;
        case 'S':  printf("stk");       break;
        case 'L':  printf("l");         break;
        default:   printf("ugyldig");   break;
    }
    printf("\n");
}
...
void skrivTilFil() {
    ...
    if(utfil){
        ...
        for(int i = 0; i < gAntallTing; i++){
            tingSkrivTilFil(utfil, gTing[i]);
        }
        printf("Lagret på TING.DTA\n");
    }else{
